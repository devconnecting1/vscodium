	...
	return (match: string, numStr: string): string => {
		...
		const varname =
			area.name === "workbench"
				...
				: `--vscode-workbench-${area.name}-font-size`;
...
async function processFile(
	...
	if (postcssResult.fails) {
		return err(
			`Failed to parse ${filePath}: ${stringifyError(postcssResult.error)}`,
		);
	}
	...
	if (generatedRoot.nodes && generatedRoot.nodes.length > 0) {
		const writeResult = await fse.writeFile(
			filePath,
	...
	postcssResult.walkRules((rule: Rule) => {
		...
		rule.walkDecls((declaration) => {
			if (PX_REGEX.test(declaration.value)) {
				...
				(declaration.prop === "height" || declaration.prop === "width")
			) {
		...
		if (declarationsToAdd.length > 0) {
			const selectors =
				rule.selectors && rule.selectors.length > 0
					? rule.selectors
					: [rule.selector];
